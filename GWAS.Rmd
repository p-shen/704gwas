...
QQPlot of PValues
```{r}
ggplot(gwaResult) + geom_qq(mapping=aes(sample=PValue), distribution = qunif) + geom_abline(intercept = 0, slope = 1)
```
...
```{r}
# allele.counts <- t(apply(control[56:60,-1:-4], 1, function(x) {
#   tab <- table(t(x))
#   if(length(tab)< 3){
#     return(NA)
#   }
#   append(dimnames(tab)[[1]], unlist(tab)) 
#   }
#   ))


test.fun <- function(x, allele) {
  tab <- table(t(x))
  print(x[allele])
  print(tab)
  ifelse(is.na(x[allele]), x[allele], 0)
}


allele.counts <- apply(control[1:10,-1:-4], 1, function(x) test.fun(x, "C T"))
