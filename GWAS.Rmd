...
gwaResult <- do.call(rbind, lapply(seq_along(1:250), GWA))
...
chisq.test(matrix(c(5511, 497, 3674, 324), nrow=2, ncol=2, byrow = T) , correct = F)$p.value
...
snps <- read.delim('imm/snps/all_snps.csv', header=F)

snps <- select(snps, V4, V5) %>% transmute(WTCCC=V4, rsid=V5)
gwaResult <- gwaResult %>% left_join(snps, by='WTCCC')


gwa10 <- read.delim('imm/gwa10.csv', sep="\t", header=T)
rswe <- gwa10[which(gwa10$WTCCC=='SNP_A-2005462'),] #rs4506565

chisq.test(
  matrix(
    as.numeric(c(rswe['ControlMajorAlleleCount'], rswe['ControlMinorAlleleCount'],
                 rswe['DiseaseMajorAlleleCount'], rswe['DiseaseMinorAlleleCount'])),
    nrow=2, ncol=2, byrow = T)
  , correct = F)$p.value


gwasResult[which(gwasResult$rsid=='rs4506565'),]
```
