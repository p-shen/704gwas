...
gwaResult <- do.call(rbind, lapply(seq_along(1:10), function(i){GWA(control[i,], disease[i,])}))
...
gwaResult$OR <- 1/gwaResult$OR
snps.all <- read.delim('snps_all', sep="\t", header=F)
...
gwaResult <- gwaResult %>%  filter(HWPValue > 0.05 & ControlMinAlleleFreq > 0.01)
...
```{r}
write.table(gwaResult, file=paste0("T2Ds.csv"), sep="\t")
```

Manhattan and QQPlot of PValues
```{r}
n <- nrow(gwaResult)
probabilities <- (1:n)/(n+1)
unif.quantiles <- qunif(probabilities, min = 0, 1)
plot(sort(unif.quantiles), sort(gwaResult$PValue) , xlab = 'Theoretical Quantiles from Uniform Distribution', ylab = 'Sample Quantiles of GWAS PValues', main = 'Uniform Quantile-Quantile Plot of GWAS PValues')
abline(0,1)
```
...
```{r}
gwaResult[which(gwaResult$rsid=='rs4506565'),]
```
